...
WHERE a2_status = 0 
GROUP BY a2_b1_id ASC LIMIT 0, 5";
...
ORDER BY b1_id ASC LIMIT 0, 6".$_REQUEST['more-matches']."";
...
<div class="modal small hide fade" id="myModal$Mecz" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<form action="system/features/change-bet.php?Mecz=$Mecz&Data_i_czas=$Data_i_czas&id_team_1=$id_team_1&id_team_2=$id_team_2" method="POST">
	...
	<table class="table list">
      <tbody>
        <tr>
          <td class="right_align">$Druzyna_1 <img src="images/flags/$Flaga_1"></td>
		  <td class="center_align">
		  <input type="text" name="Wynik_1" value="" maxlength="2" size="2" class="span3" pattern="[0-9]{1,2}" required placeholder="?"> - 
		  <input type="text" name="Wynik_2" value="" maxlength="2" size="4" class="span3" pattern="[0-9]{1,2}" required placeholder="?">
		  </td>
          <td><img src="images/flags/$Flaga_2"> $Druzyna_2</td>
        </tr>
      </tbody>
...
ORDER BY a2_b1_id ASC LIMIT 0, 6".$_REQUEST['more-bets']."";
$result = mysql_query($query) or die(mysql_error());
...
while ($row = mysql_fetch_array($result)) {
	...
	$efficacy = round((($sum_a2_points/($count_a2_b1_id*3))*100), 2);
...
inner join b_team t2 on t2.b_id = a2_bets.a2_team_2 
WHERE a2_a_id = '".$_SESSION['user_id']."' AND a2_status != 0 
ORDER BY a2_b1_id DESC LIMIT 0, 6".$_REQUEST['more-history']."";
$result = mysql_query($query) or die(mysql_error());
